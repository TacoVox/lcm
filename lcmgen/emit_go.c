        ...
        if (dim->mode == LCM_VAR) {
            ...
            if (slice_emit)
                emit_go_slice_make(f, n + 1, ls->structname->package, lm, n,
                    slicestr->str, size, lcm_get_fingerprint(lcm, ls));
    ...
    for (unsigned int k = 0; k < ls->members->len; k++) {
        ...
        for (;
            ...
            for (; j < lm_->dimensions->len;
                ...
                GString *arraystr = g_string_new(NULL);
                emit_go_array_loops(f, lcm, ls, lm_, arraystr, FALSE, j);
    ...
    int64_t fingerprint)
{
    ...
    char *gopackage = strip_dots(ls->package);
    char *gotype = go_typename(ls->package, ls->structname->package,
    ...
    // Const definitions
    emit_go_lcm_const_definitions(f, lcm, ls, gotype);

    // Struct definition
    if (emit_go_lcm_struct_definition(f, lcm, ls, gotype) != 0) {
        res = -1;
        goto ret_file;
    }
    ...
    // Functions
    // Deep copy
    emit_go_lcm_deep_copy(f, lcm, ls, gotype);

    // Encode
    emit_go_lcm_encode(f, lcm, ls, gotype);

    // MarshalBinary
    emit_go_lcm_marshal_binary(f, lcm, ls, gotype);

    // Decode
    emit_go_lcm_decode(f, lcm, ls, gotype);

    // UnmarshalBinary
    emit_go_lcm_unmarshal_binary(f, lcm, ls, gotype);

    // Getters for read only fields (all fields used as array sizes)
    emit_go_lcm_read_only_getters(f, lcm, ls, gotype);

    // Fingerprint
    if (fingerprint == 0) {
        emit_go_lcm_fingerprint(f, lcm, ls, typename, gotype);
    }

    // Size
    emit_go_lcm_size(f, lcm, ls, gotype);
    ...
    int64_t fingerprint)
{
    ...
    char *gopackage = strip_dots(ls->package);
    char *gotype = go_typename(ls->package, ls->structname->package,
        ls->structname->lctypename, lcm_get_fingerprint(lcm, ls));
    ...
    emit_start(1, "layerClass%s = gopacket_lcm.RegisterLCMLayerType(",
        gotype);
    emit_continue_go_fingerprint_string(f, lcm, gotype);
