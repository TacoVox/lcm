...
#define emit(n, ...) \
    do { fprintf(f, "%*s", INDENT(n), ""); fprintf(f, __VA_ARGS__); fprintf(f, "\n"); } while (0)
#define emit_nl() \
    do { fprintf(f, "\n"); } while (0)
...
/*static const char * primitive_type_to_arr(const char *type, int index) {
    size_t len = strlen(type);
    char *newStr = (char *)malloc(len + 5);

    sprintf(newStr, "%s[i%d]", type, index);
...
{
    if (!strcmp(t,"boolean")) return strdup("bool");
    if (!strcmp(t,"byte")) return strdup("byte");
    if (!strcmp(t,"int8_t")) return strdup("int8");
    if (!strcmp(t,"int16_t")) return strdup("int16");
    if (!strcmp(t,"int32_t")) return strdup("int32");
    if (!strcmp(t,"int64_t")) return strdup("int64");
    if (!strcmp(t,"float")) return strdup("float32");
    if (!strcmp(t,"double")) return strdup("float64");
    if (!strcmp(t,"string")) return strdup("string");
    ...
    int indent)
{
    if (!strcmp(type, "boolean")) {
        ...
        emit(indent, "p.%s = uint8(data[offset])", dst);
        emit(indent, "offset += 1");
    } else if (!strcmp(type, "int16_t")) {
        emit(indent, "p.%s, _ = binary.BigEndian.Uint16(data[offset:])", dst);
        emit(indent, "offset += 2");
    } else if (!strcmp(type, "int32_t")) {
        emit(indent, "p.%s, _ = binary.BigEndian.Uint32(data[offset:])", dst);
        emit(indent, "offset += 4");
    } else if (!strcmp(type, "int64_t")) {
        emit(indent, "p.%s, _ = binary.BigEndian.Uint64(data[offset:])", dst);
        emit(indent, "offset += 8");
    } else if (!strcmp(type, "float")) {
        emit(indent, "p.%s, _ = binary.BigEndian.Uint32(data[offset:])", dst);
        emit(indent, "offset += 4");
...
static void emit_go_deep_copy(FILE *f, lcm_struct_t *ls, const char *gotype) {
    ...
    emit(0, "func (p *%s) Copy() (%s) {", gotype, gotype);
    emit(1, "copy := %s{}", gotype);
    emit_nl();
    ...
    for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
        ...
        if (lcm_is_primitive_type(lm->type->lctypename)) {
            ...
            if (! lm->dimensions->len) {
                emit(1, "copy.%s = p.%s", membername, membername);
            } else {
                unsigned int n;
                ...
                emit(n + 1, "copy.%s = p.%s.Copy()", arraystr->str, arraystr->str);
            ...
            if (! lm->dimensions->len) {
                emit(1, "copy.%s = p.%s.Copy()", membername, membername);
            } else {
                unsigned int n;
                ...
                emit(n + 1, "copy.%s%s = p.%s%s.Copy()", membername,
...
static void emit_go_marshal_binary(FILE *f, lcm_struct_t *ls, const char *gotype) {
    ...
    for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
        ...
        if (lcm_is_primitive_type(lm->type->lctypename)) {
            if (! lm->dimensions->len) {
                emit_conversion_function(f, lm->type->lctypename, membername, 1);
            } else {
                unsigned int n;
                GString *arraystr = g_string_new(membername);
                for (n=0; n<lm->dimensions->len; n++) {
                ...
                emit_conversion_function(f, lm->type->lctypename, arraystr->str, 1+n);
                g_string_free(arraystr, TRUE);
...
static void emit_go_unmarshal_binary(FILE *f, lcm_struct_t *ls, const char *gotype) {
    ...
    emit(0, "func (p *%s) UnmarshalBinary(data []byte) (int, error) {", gotype);
    emit(1, "offset := 0");
    emit_nl();
    ...
    for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
        ...
        if (lcm_is_primitive_type(lm->type->lctypename)) {
            ...
            if (! lm->dimensions->len) {
                emit(1, "o, e := p.%s.UnmarshalBinary(); if e != nil {", membername);
                emit(2, "return offset, e");
                emit(1, "}");
                emit(1, "offset += o");
            } else {
                unsigned int n;
                ...
                emit(n + 1, "o, e := p.%s%s.UnmarshalBinary(data[offset:]); if e != nil {", membername,
                    arraystr->str);
                emit(n + 2, "return offset, e");
                emit(n + 1, "}");
                emit(n + 1, "offset += o");
                g_string_free(arraystr, TRUE);
...
static void emit_go_size(FILE *f, lcm_struct_t *ls, const char *gotype) {
    ...
    if (!g_ptr_array_size(ls->members)) {
        ...
        for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
            ...
            if (m < g_ptr_array_size(ls->members) - 1) {
                emit_end(" +");
                emit_start(1, "");
            } else {
                emit_end("");
